...
public class MinecraftAccount
{
    ...
    void updateSkinFace(String uuid) {
        try {
            File skinFile = File.createTempFile("skin", ".png", new File(AccountTools.DIR_DATA, "cache"));
            AccountTools.downloadFile("https://mc-heads.net/head/" + uuid + "/100", skinFile.getAbsolutePath());
    ...
    public String save(String outPath) throws IOException {
        AccountTools.write(outPath, AccountTools.GLOBAL_GSON.toJson(this));
        return username;
    }

    public String save() throws IOException {
        return save(AccountTools.DIR_ACCOUNT_NEW + "/" + username + ".json");
    }

    public static MinecraftAccount parse(String content) throws JsonSyntaxException {
        return AccountTools.GLOBAL_GSON.fromJson(content, MinecraftAccount.class);
    }
    ...
    public static MinecraftAccount load(String name) throws JsonSyntaxException {
        ...
        try {
            MinecraftAccount acc = parse(AccountTools.read(AccountTools.DIR_ACCOUNT_NEW + "/" + name + ".json"));
    ...
    private static boolean accountExists(String username){
        return new File(AccountTools.DIR_ACCOUNT_NEW + "/" + username + ".json").exists();
    }

    public static void clearTempAccount() {
        File tempAccFile = new File(AccountTools.DIR_DATA, "cache/tempacc.json");
        tempAccFile.delete();
    }
